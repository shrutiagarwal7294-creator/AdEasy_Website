/* AdsEasy Media — Real Estate: Cost per Lead Calculator
 * Self-contained script scoped to the #cpl-calculator widget.
 * Deliberately independent from assets/js/calculators.js (tools.html)
 * so this page can't affect that one, or vice versa.
 */
(function () {
  'use strict';
  ...
  function calc() {
    ...
    var spend = num('cpl-spend', 150000);
    var leads = Math.max(num('cpl-leads', 120), 0);
  ...
  function waLines(fields) {
    ...
    var cpl = leads ? spend / leads : 0;
    var lines = [
      'Hi AdsEasy Media! I used the Cost per Lead calculator on your Real Estate Industry page.',
      ''
    ];
    if (fields) lines.push('Name: ' + fields.name, 'Email: ' + fields.email, 'Phone: ' + fields.phone, '');
    lines.push(
      'Monthly spend: ' + money(spend),
      'Leads generated: ' + fmt(leads),
