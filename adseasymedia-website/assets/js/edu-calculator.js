/* AdsEasy Media — Education: Cost per Lead Calculator
 * Self-contained script scoped to the #cpl-calculator widget on
 * education-marketing.html. Deliberately independent from
 * assets/js/calculators.js (tools.html) and assets/js/re-calculator.js
 * (real estate) so none of these pages can affect each other.
 */
(function () {
  'use strict';
  ...
  function calc() {
    ...
    var spend = num('cpl-spend', 100000);
    var leads = Math.max(num('cpl-leads', 200), 0);
  ...
  function waLines(fields) {
    ...
    var cpl = leads ? spend / leads : 0;
    var lines = [
      'Hi AdsEasy Media! I used the Cost per Lead calculator on your Education Industry page.',
      ''
    ];
    if (fields) lines.push('Name: ' + fields.name, 'Email: ' + fields.email, 'Phone: ' + fields.phone, '');
    lines.push(
      'Monthly spend: ' + money(spend),
      'Admissions inquiries generated: ' + fmt(leads),
